function genPerfMat(datasetPath, seqs, trackers, evalType, dataType, rpAll, perfMatPath, thresholdSetOverlap, thresholdSetError, nameTrkAll)
...
for idxSeq=1:length(seqs)
    ...
    for idxTrk = 1:numTrk       
        t = trackers{idxTrk};        
        if strcmp(t.name, 'ASNet') || strcmp(t.name, 'DSiam_RD_VAF') || strcmp(t.name, 'DSiam_TS_VAF') || strcmp(t.name, 'DSiam_VAF')
            continue
        end
        % check the result format
        res_mat = [rpAll s.name '_' t.name '.mat'];
        if(~exist(res_mat, 'file'))
            res_txt = [rpAll s.name '_' t.name '.txt'];
            results = {};
