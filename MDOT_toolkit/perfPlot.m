...
for i = 1:length(metricTypeSet)
    ...
    if show_IFS
        ...
        if(~exist(dataName, 'file') || reEvalFlag)
            genPerfMat(datasetPath, seqs, trackers, evalType, dataType, rpAll, perfMatPath, thresholdSetOverlap, thresholdSetError, nameTrkAll);
        end
    end
...
%     if show_IFS == 1
%         figName_IFS = [figPath 'IFS_' plotType '_' rankingType];
%         switch metricType
%             case 'overlap'
%                 titleName_IFS = ['Success plots of IFS' showType];
%             case 'error'
%                 titleName_IFS = ['Precision plots of IFS' showType];
%         end
%         plotDrawSave(numTrk,plotDrawStyle,aveSuccessRatePlot,idxSeqSet,rankNum,rankingType,rankIdx,nameTrkAll,thresholdSet,titleName_IFS, xLabelName,yLabelName,figName_IFS);
%     end
