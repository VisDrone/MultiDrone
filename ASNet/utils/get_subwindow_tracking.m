% --------------------------------------------------------------------------------------------------------
function [im_patch, im_patch_original] = get_subwindow_tracking(im, pos, model_sz, original_sz, avg_chans,avgimg)
...
%             im_patch = gpuArray(mexResize(gather(im_patch_original), model_sz, 'auto'));
    ...
    if ~isempty(avgimg)
        im_patch = im_patch-imresize(avgimg,[size(im_patch,1),size(im_patch,2)]);
    end
end
