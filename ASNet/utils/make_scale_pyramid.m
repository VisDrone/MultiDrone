...
% -----------------------------------------------------------------------------------------------------
    ...
    [search_region, ~] = get_subwindow_tracking(gather(im), targetPosition, [search_side search_side], [max_target_side max_target_side], avgChans);

    assert(round(beta * min_target_side)==out_side);

    for s = 1:p.numScale
        target_side = round(beta * in_side_scaled(s));
        pyramid(:,:,:,s) = get_subwindow_tracking(search_region, (1+search_side*[1 1])/2, [out_side out_side], target_side*[1 1], avgChans,p.averageImage);
    end
end