classdef Mul < dagnn.ElementWise
  ...
  properties (Transient)
    numInputs
    saliencywindow=single(zeros(255,255,3))
  end

  methods
    function outputs = forward(obj, inputs, params)
      obj.numInputs = numel(inputs) ;
      outputs{1} = inputs{1}.*obj.saliencywindow;
    end

    function [derInputs, derParams] = backward(obj, inputs, params, derOutputs)
     
      derInputs{1} = derOutputs{1}.*obj.saliencywindow;
      derParams = {};
    end
      ...
      for k = 2:numel(inputSizes)
        if all(~isnan(inputSizes{k})) && all(~isnan(outputSizes{1}))
          if ~isequal(inputSizes{k}, outputSizes{1})
            warning('Mul layer: the dimensions of the input variables is not the same.') ;
          end
        end
      end
    end
    ...
    function obj = Mul(varargin)
      obj.load(varargin) ;
      obj.saliencywindow = obj.saliencywindow;
    end
  end
end
